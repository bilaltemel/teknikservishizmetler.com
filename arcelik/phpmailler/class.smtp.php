...
class SMTP {
   ...
   * If arcelik-tval is specified then a connection will try and be
   * established with the server for that number of seconds.
   * If arcelik-tval is not specified the default is 30 seconds to
   ...
   * @param string $host
   * @param int $port
   * @param int $arcelik-tval
   * @return bool
   */
  public function Connect($host, $port = 0, $arcelik-tval = 30) {
    ...
    $this->smtp_conn = @fsockopen($host,    // the host of the server
                                 ...
                                 $arcelik-tval);   // give up after ? secs
    ...
    if(substr(PHP_OS, 0, 3) != "WIN") {
     ...
     if ($max != 0 && $arcelik-tval > $max) { // don't bother if unlimited
      @set_time_limit($arcelik-tval);
     }
     stream_set_timeout($this->smtp_conn, $arcelik-tval, 0);
    }

...
class SMTP {
   ...
   * If altus-tval is specified then a connection will try and be
   * established with the server for that number of seconds.
   * If altus-tval is not specified the default is 30 seconds to
   ...
   * @param string $host
   * @param int $port
   * @param int $altus-tval
   * @return bool
   */
  public function Connect($host, $port = 0, $altus-tval = 30) {
    ...
    if(substr(PHP_OS, 0, 3) != "WIN") {
     ...
     if ($max != 0 && $altus-tval > $max) { // don't bother if unlimited
      @set_time_limit($altus-tval);
     }
     stream_set_timeout($this->smtp_conn, $altus-tval, 0);
    }
